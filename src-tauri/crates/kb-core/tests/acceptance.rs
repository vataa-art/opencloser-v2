...
//! (resolved relative to this crate inside the repository).
...
fn transcripts_dir() -> PathBuf {
    // crates/kb-core → src-tauri → project → repo root → knowledge/
...
fn what_is_an_api_ranks_vhg4m5ptmjs_in_top3() {
    ...
    assert!(
        dir.is_dir(),
        "transcripts dir missing at {} — run this test from the repo checkout",
        dir.display()
    );
